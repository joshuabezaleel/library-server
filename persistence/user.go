...
func (repo *userRepository) Save(user *user.User) (*user.User, error) {
	_, err := repo.DB.NamedExec("INSERT INTO users (id, student_id, role, username, email, password, registered_at) VALUES (:id, :student_id, :role, :username, :email, :password, :registered_at)", user)
...
func (repo *userRepository) Update(user *user.User) (*user.User, error) {
	_, err := repo.DB.NamedExec("UPDATE users SET student_id=:student_id, role=:role, username=:username, email=:email, password=:password WHERE id=:id", user)
